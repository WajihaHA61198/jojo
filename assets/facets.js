(function ($) {
  ...
  var Facets = (function () {
    var PageURL,
      ...
      activeFilterSelector = '.sr4-active-filters a',
      ...
      isSearchOrCollection = sr4request.page_type == 'collection' || sr4request.page_type == 'search',
      //loading              = 'is--loading',
    ...
    Facets.prototype = Object.assign({}, Facets.prototype, {
      ...
      closeDrawer: function ($this) {
        $this.closest('.sr4-drawer').find('[data-drawer-close]').trigger('click.drawer');
      },
      ...
      setupEventListenersSidebarFacets: function () {
        ...
        var $wrapperPrice = self.$filterSidebar.find('.sr4-price_slider_wrapper');

        if ($wrapperPrice.length == 0) return;
        $script(sr4configs.script8, function () {
      ...
      setupEventListenersFacets: function () {
        ...
        var $wrapperPrice = self.$filter.find('.sr4-price_slider_wrapper'),
          $wrapperPriceArea = self.$filterArea.find('.sr4-price_slider_wrapper');
      ...
      setupEventListeners: function () {
        ...
        // self.$container.on('sr4:loadmore', function(e) {
        // 	self.clickOnScrollButton();
        // });
        //self.$container.on('sr4:loadmore', self.clickOnScrollButton );
      ...
      loadPageContent: function (page) {
        ...
        self.renderSection(page, self.main, function (data, id, filterUrl, isFetch) {
          ...
          // Update Content main collection
          var $elsObservered = self.$container.find(
            dataResizeobserver +
              '.flickitysr4-enabled .sr4-product.sr4_observered, ' +
              dataResizeobserver +
              '.isotopesr4-enabled .sr4-product.sr4_observered'
          );
          ...
          self.updateListing();
          if (self.isProduct) sr4ThemeSP.reinitProductGridItem();
          sr4ThemeSP.PopupMFP();
          $body.trigger('sr4:hideTooltip');
          $body.trigger('currency:update');

          if (self.typeAjax == 'AjaxIsotope' || self.typeAjax == 'LmIsotope') {
            sr4ThemeSP.Isotopesr4.init(self.$container.find('.isotopesr4'));
          ...
          self.renderSection(page, self.filter, function (data, id, filterUrl, isFetch) {
            ...
            if (self.nouisliderLoaded) {
              self.initPriceRange(self.$filter.find('.sr4-price_slider_wrapper'));
              self.initPriceRange(self.$filterSidebar.find('.sr4-price_slider_wrapper'));
              self.initPriceRange(self.$filterArea.find('.sr4-price_slider_wrapper'));
            }
            self.removeDataDelay();
          });
        });
      ...
      // const elementSelectors = ['.sr4-active-filters', '.sr4-products', '.sr4-pagination-wrapper'];
      ...
      initPriceRange: function (_this) {
        var self = this,
          _amount = _this.find('.sr4-price_slider_amount'),
          minstr = _amount.data('minstr'),
          maxstr = _amount.data('maxstr'),
          _minPrice = _amount.find('.sr4-min_price'),
          _maxPrice = _amount.find('.sr4-max_price'),
          currentUrlPrice = _amount.find('.sr4-url_price').val(),
        ...
        _amount.find('input.sr4-url_price, input.sr4-min_price, input.sr4-max_price').hide();
        _this.find('.sr4-price_slider, .sr4-price_label ,.sr4-price_slider_btn').show();

        var $stepsSlider = _this.find('.sr4-price_steps_slider:not(.noUi-target)'),
          stepsSlider = $stepsSlider[0],
          inputs = ['span.sr4-from', 'span.sr4-to'],
          inputs2 = ['.sr4-min_price', '.sr4-max_price'],
          check_dis = true;
        ...
        stepsSlider.noUiSlider.on('update', function (values, handle) {
          ...
          _this.find(inputs[handle]).html(sr4ThemeSP.Currency.formatMoney(value_curent));
          //_amount.find(inputs2[handle]).val(value_curent);
          _amount
            .find(inputs2[handle])
            .val(sr4ThemeSP.Currency.formatMoney(value_curent, '{{amount}}').replaceAll(',', ''));
        ...
        _this.on('click', '.sr4-price_slider_btn', function (e) {
          currentMinPrice = _minPrice.val();
          currentMaxPrice = _maxPrice.val();
      ...
      layoutSwitching: function (obj, key, value) {
        var self = this,
          ...
          cart_update_url = sr4routes.cart_update_url;
        ...
        self.$section.on('click', str_switch + ' button', function (e) {
          ...
          var $this = $(this),
            ...
            col = $this.data('col'),
            $products = self.$container.find('.sr4-products');
          ...
          if (breakpoint == 'mobile') {
            if (!action_load) {
              $products.removeClass('sr4-row-cols-' + col_mb).addClass('sr4-row-cols-' + col);
            }
            col_mb = col;
          } else if (breakpoint == 'tablet') {
            if (!action_load) {
              $products.removeClass('sr4-row-cols-md-' + col_tb).addClass('sr4-row-cols-md-' + col);
            }
            col_tb = col;
          } else {
            if (!action_load) {
              $products.removeClass('sr4-row-cols-lg-' + col_dk).addClass('sr4-row-cols-lg-' + col);
            }
            col_dk = col;
          }

          // update layout isotopesr4 if exit
          if ($products.hasClass('isotopesr4')) {
            $products.isotopesr4('layout');
          }
        ...
        $window.on('resize', function () {
          ...
          var $active_visible = $switch.find('.is--active:visible'),
            ...
            $products = $switch.find('.sr4-products');
      ...
      updateListing: function () {
        ...
        if (this.$section.find('.sr4_icon_viewlist').length == 0) return;

        var windowWidth = window.innerWidth,
          //dataSwitch      = cacheSwitch || this.$section.find('[data-layout-switch]').attr('data-items'),
          onlistview = cacheSwitch.split('.'),
          $product = this.$container.find('.sr4-products'),
          classListview = 'is--listview on--list-',
          classListview2 = classListview;

        // console.log(onlistview,$product\
        // sr4-row-cols-2 sr4-row-cols-md-3 sr4-row-cols-lg-6
        if (cacheSwitch) {
          const arrNeedRemove = [...$product[0].classList].filter((cl) => /sr4-row-cols-/g.test(cl));
          $product
            .removeClass(arrNeedRemove.join(' '))
            .addClass(
              `sr4-row-cols-${onlistview[0]} sr4-row-cols-md-${onlistview[1]} sr4-row-cols-lg-${onlistview[2]}`
            );
        }

        if (windowWidth < 768 && onlistview[0] == 'list_sr4') {
          classListview = classListview + 'mobile';
        } else if (windowWidth < 1025 && onlistview[1] == 'list_sr4') {
          classListview = classListview + 'tablet';
        } else if (onlistview[2] == 'list_sr4') {
      ...
      onFirstLoadFacets: function () {
        if (IsDesignMode) {
          var self = this,
            ...
            $filter = $('#sr4-filter-hidden.is--sr4-filter-native'),
            $filterTags = $('#sr4-filter-hidden.is--sr4-filter-tags'); // shopify:section:deselect
          ...
          // Upate button filter
          $('.sr4-toolbart-filter > a')
            .removeAttr('data-drawer-options')
            .attr('data-drawer-options', `{ "id":"#shopify-section-${self.filter} #sr4-filter-hidden" }`);
          ...
          $document.on(SectionLoadEvt, function (e) {
            ...
            if ($parentSection.hasClass('sr4-section-filter')) {
              if ($parentSection.hasClass('sr4-section-sidebar')) {
                // console.log('ttt2')
              ...
              // } else if ($parentSection.hasClass('sr4-section-main') && ( $parentSection.find('[data-replace-filter]').length > 0 || self.$filterArea.length > 0 ) ) {
            } else if ($parentSection.hasClass('sr4-section-main')) {
              ...
              $('.sr4-toolbart-filter > a')
                .removeAttr('data-drawer-options')
                .attr('data-drawer-options', `{ "id":"#shopify-section-${self.filter} #sr4-filter-hidden" }`);
              //console.log( $('.sr4-toolbart-filter').attr('data-drawer-options', `{ "id":"#shopify-section-${self.filter} #sr4-filter-hidden" }`) )
              //$('.sr4-toolbart-filter')

              //self.$filterArea = $parentSection.find('[data-filter-area]');
              //console.log($parentSection,'yy')
              let htmlFilter =
                self.options.typeFilters == str_facets
                  ? $filter.find('#FacetFiltersForm').parent().html()
                  : $filterTags.find('#FacetFiltersForm').parent().html();
              ...
              if ($parentSection.find('[data-replace-filter]').length > 0 || self.$filterArea.length > 0)
                self.setupEventListenersFacets();
          ...
          $document.on(SectionSelectEvt, function (e) {
            ...
            $filter = $('#sr4-filter-hidden.is--sr4-filter-native');
            $filterTags = $('#sr4-filter-hidden.is--sr4-filter-tags');

            if (id != str_facets && id != str_facets_tags) {
              if (
                ((self.$filterArea.length > 0 && $window.width() > 767) || !isMobileOffFilterArea) &&
                $parentSection.hasClass('sr4-section-main')
              ) {
                self.$filterArea.html(
                  self.options.typeFilters == str_facets
                    ? $filter.find('#FacetFiltersForm').parent().html()
                    : $filterTags.find('#FacetFiltersForm').parent().html()
                );
            ...
            if ((self.$filterArea.length > 0 && $window.width() > 767) || !isMobileOffFilterArea) {
              ...
              sr4ThemeSP.Drawer.opend($filter);
            } else if (id == str_facets_tags && $filterTags.length > 0) {
              sr4ThemeSP.Drawer.opend($filterTags);
            }
          });
          ...
          $document.on(SectionDeselectEvt, function (e) {
            ...
            if ((self.$filterArea.length > 0 && $window.width() > 767) || !isMobileOffFilterArea) {
              ...
              $('.sr4-close-overlay:visible').trigger('click');
            }
          });
      ...
      onFirstLoadFilterSidebar: function () {
        ...
        if (filterDataSidebar != 'nosr4' && self.$filterSidebar.length > 0) {
          self.$filterSidebar.html(filterDataSidebar);
          filterDataSidebar = '';
          self.setupEventListenersSidebarFacets();
        } else {
      ...
      onFirstFetchFilter: function (isCahecData = true, idSection) {
        ...
        if (filterFetchStatus == '') {
          ...
          fetch(
            `${pathname}/?${queryParamsStr}${prefix}section_id=${IsDesignMode && idSection ? idSection : self.filter}`,
            {
              method: 'GET',
              headers: { 'Cache-Control': 'no-cache' },
            }
          ...
          $document.on('facets:loaded', function (event, _isCahecData, _response) {
            ...
            if (_isCahecData) {
              sr4ThemeSP.$appendComponent.after(_response);
      ...
      removeDataDelay: function () {
        ...
        $('[data-drawer-options*="#sr4-filter-hidden"]').removeAttr(dataDrawerDelay);
      },
      ...
      clickOnScrollButton: function () {
        var self = this,
          $els = self.$container.find('[data-load-onscroll]:not(.sr4-lm-onscroll-init)');
        ...
        $els.each(function (index) {
          ...
          $(this).addClass('sr4-lm-onscroll-init');
        });
      },
      ...
      //    $content.html(items);
      //    this.$section.find(str_ntjax_wrap).html( paginationHTML );
      //    $content.isotopesr4('destroy');
      //    sr4ThemeSP.Isotopesr4.init($content);
  ...
  var onlyOne = false;
  sr4ThemeSP.initFacets = (function () {
    // console.log('initFacets')
    $('.sr4-section-main [' + dataNtajaxOptions + ']:not(.is--enabled)').each(function () {
      // console.log('xxx')
  ...
  if (IsDesignMode && !onlyOne) {
    $document.on(SectionLoadEvt, function (e) {
      ...
      if (!$parentSection.hasClass('sr4-section-main') || onlyOne) return;
